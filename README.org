...
By default guess-language only switches the Ispell dictionary used by Flyspell (`ispell-change-dictionary`).  However, guess-language can also switch the language settings used by type-mode.  To activate this add the following to your configuration:

#+BEGIN_SRC elisp
(add-hook 'guess-language-after-detection-functions 'guess-language-switch-typo-mode-function)
#+END_SRC
